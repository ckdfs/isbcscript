...
| 占空比 $A$ | `config.PWM_DUTY_CYCLE`（默认 0.5） |
| $\phi_0$ | $\arctan((1-A)/A)$（默认 $\pi/4 = 45°$） |
| $\kappa$ | $\sqrt{A^2+(1-A)^2}$（默认 $1/\sqrt{2}$，$-3\ \text{dB}$） |
| 拟合模型 | $r = A_{fit}\cdot\|\tan(\phi_0 - \pi(v-V_0)/V_\pi^{fit})\|$（随 $v$ 单调递减） |
| $R_{target}$ | $A_{fit}\tan\phi_0$（默认占空比 0.5 时等于 $A_{fit}$） |
| CH1 模式 | ARB（预加载波形） |
...
> `max_quad` 是当前唯一仍使用比值 PI 控制的模式。占空比可通过
> `config.PWM_DUTY_CYCLE` 调整；改变占空比后，`compute_r_target()` 和
> `adjust_k_i()` 会按 $\phi_0$ 自动修正目标比值和积分增益。
...
| `offset_limits` | $[\max(-6.9, V_\pi/2-2V_\pi),\; \min(6.9, V_\pi/2+V_\pi)]$ |
...
> 当前实验流程只选择仪器上已有 ARB，不在 `configure_source()` 中重新上传波形。
...
| `initial_offset` | 谷底 offset（由 quick_estimate 从扫描数据确定） |
| `sweep_offsets` | base + $V_\pi/2$ |
| `offset_limits` | $[\max(-10+\tfrac{V_\pi+0.8}{2},\; V_\pi/2-2V_\pi),\; \min(10-\tfrac{V_\pi+0.8}{2},\; V_\pi/2+V_\pi)]$ |
