...
quad_pm 的目标工作点是两态分别在正/负正交点：
- 状态B：$\phi_{DC} = \pi/2$（正正交点）
- 状态A：$\phi_{DC} + \pi = 3\pi/2$（负正交点）

此时 $S_2 \propto 2J_2|\cos\phi_{DC}| = 2J_2|\cos(\pi/2)| = 0$。

**$S_2$ 在目标点恰好为零（谷底）**，所以控制策略很简单：直接追 $S_2 \to \min$。

### 4.2 与比值法的对比

比值法 $r = \kappa(J_1/J_2)|\tan\phi_{DC}|$ 在 $\phi_{DC}\to\pi/2$ 时 $r\to\infty$（渐近线）。这导致：
- curve_fit 拟合窗口包含渐近线 → 初始猜测爆炸 → 拟合发散
- PI 控制的目标 $R_{target}$ 在渐近线附近不可达

而 $S_2 \propto |\cos\phi_{DC}|$ 在 $\phi_{DC}=\pi/2$ 处是最小值，天然适合做控制目标。

### 4.3 实现：自适应探针梯度下降

$S_2$ 在谷底附近梯度极大（$\sim 100$ dB/V），远离谷底时梯度平缓（$\sim 5$ dB/V）。
固定探针无法兼顾：太大会在谷底附近反复过冲，太小在远处无法判断方向。

解决方案——探针步长自适应缩放：

```
初始点：扫描数据中 S₂ 最低点对应的 offset（≈ Vπ/2）
每轮迭代：
  1. 测当前 S₂ → 算 excess_dB = S₂ − S₂_floor
  2. 探针 = max(0.02, min(0.10, excess_dB / 50))  ← 自适应
  3. 正向探测 → 测 S₂_probe → 判断方向
  4. 步长 = 0.003 × excess_dB → 限幅、限位
  5. 死区：步长 < 0.002 V 时不动
```

| 区域 | excess_dB | 探针 | S₂ 梯度 | 步长 |
|------|-----------|------|---------|------|
| 远离谷底 | 30 dB | 0.10 V | ~5 dB/V | 0.09 V |
| 中距离 | 10 dB | 0.10 V | ~50 dB/V | 0.03 V |
| 近谷底 | 3 dB | 0.06 V | ~100 dB/V | 0.009 V |
| 谷底 | 0 dB | 0.02 V | — | 0 (死区) |

### 4.4 实现约定

- `vdc_ref = Vpi/2`（两正交点电气中点，即 S₂ 谷底位置）
- `sweep_offsets = base + Vpi/2`（扫描中心在谷底附近）
- `control_strategy = 's2_min'`（自适应探针梯度下降）
- `use_curve_fit = False`（vdc_eff=0 在渐近线，拟合不可靠）
- `offset_limits` 上限 6.9 V（受 ARB 振幅 6.2 Vpp 的硬件限制）
