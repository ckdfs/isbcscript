...
每轮迭代：
  1. 测当前 S₂
  2. 正向探测 +0.02V → 测 S₂_probe
  3. 若 S₂_probe < S₂ → 正向走（往谷底）
     若 S₂_probe > S₂ → 反向走
  4. 步长 ∝ (S₂ − S₂_floor)，离谷底越远步子越大
  5. 死区：|S₂ − S₂_floor| < 2 dB 时不动
```
...
扫描偏移 `sweep_offsets = base + Vpi/2`，使扫描中心在 S₂ 谷底附近。

> 注意：`vdc_ref = Vpi/2` 使 `vdc_eff = 0` 正好在渐近线位置。
> 因此 quad_pm **不使用比值法拟合**，`--step fit` 自动跳过 curve_fit。
