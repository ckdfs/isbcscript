...
实际测得的稳态频谱仪读数差（$S_1 - S_2$）即为 $20\log_{10}r$，可直接从控制日志验证。

| | $r$ | dB |
|--|--|--|
| 理论值（$V_\pi = 5.4\,\text{V}$） | $\approx 17.15$ | 24.69 dB |
| 理论值（$V_\pi = 5.253\,\text{V}$，拟合值） | $\approx 16.68$ | 24.44 dB |
| **实验拟合值**（`fit_result.json`） | **18.71** | **25.44 dB** |
| 差距 | | $+0.76 \sim +1.00\,\text{dB}$ |

实验值略高于理论值，差距约 **+0.76–1.0 dB**：

$$r_{meas} = \underbrace{\frac{J_1}{J_2}}_{\approx 17.15} \times \underbrace{\frac{H(20\,\text{kHz})}{H(40\,\text{kHz})}}_{\approx 1.09\;(+0.76\,\text{dB})}$$

即系统在 20 kHz 处的增益仅比 40 kHz 高约 **0.76–1.0 dB**，两频点之间的响应几乎完全平坦。这一小偏差处于 $V_\pi$ 测量误差和导频幅度误差的正常范围内，说明系统行为与理论高度吻合。

> 尽管如此，仍建议通过曲线拟合（而非直接使用理论值）获取 $R_{target}$：
> 拟合所得 $A_{fit}$ 自动包含仪器校准偏差、$V_\pi$ 测量误差等所有系统效应，
> 更加精确可靠。
